...
__all__ = ['MobileNetV2_34', 'mobilenetv2_34']
...
class MobileNetV2_34(nn.Module):
    def __init__(self, block, layers, num_classes=1000):
        self.inplanes = 32
        super(MobileNetV2_34, self).__init__()
        self.conv1 = nn.Conv2d(3, 32, kernel_size=3, stride=2, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(32)
        self.relu = nn.ReLU(inplace=True)
        self.layer1 = self._make_layer(block, 16, layers[0], stride=1, expansion=1)
        self.layer2 = self._make_layer(block, 24, layers[1], stride=2, expansion=5)
        self.layer3 = self._make_layer(block, 32, layers[2], stride=2, expansion=5)
        self.layer4 = self._make_layer(block, 64, layers[3], stride=1, expansion=5)
        self.layer5 = self._make_layer(block, 96, layers[4], stride=2, expansion=5)
        self.layer6 = self._make_layer(block, 160, layers[5], stride=2, expansion=4)
        self.layer7 = self._make_layer(block, 320, layers[6], stride=1, expansion=4)
...
def mobilenetv2_34(**kwargs):
    """Constructs a MobileNetV2-19 model.
    """
    model = MobileNetV2_34(Bottleneck, [1, 1, 2, 3, 4, 3, 3], **kwargs)
    return model
